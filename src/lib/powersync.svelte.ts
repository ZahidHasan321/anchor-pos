...
export class PowerSyncManager {
	...
	private async _checkCachedData() {
		try {
			const count = (await this.db.get('SELECT COUNT(*) as cnt FROM ps_oplog')) as {
				cnt: number;
			} | null;
	...
	async connect(providedUserId?: string) {
		...
		try {
			...
			if (!powersyncUrl) {
				throw new Error(
					'PowerSync URL not configured. Set VITE_POWERSYNC_URL in your environment.'
				);
			}
			...
			const connector: any = {
					...
					for (let attempt = 1; attempt <= maxAttempts; attempt++) {
						try {
							...
							if (!res.ok) {
								...
								console.error(
									`[PowerSync] Token fetch failed (attempt ${attempt}/${maxAttempts}): ${res.status} ${body}`
								);
							...
							console.error(
								`[PowerSync] Credential fetch error (attempt ${attempt}/${maxAttempts}):`,
								e
							);
							if (attempt < maxAttempts) {
								await new Promise((r) => setTimeout(r, 1000 * attempt));
				...
				uploadData: async (database: any) => {
					...
					try {
						// Upload to VPS — session cookie provides auth
