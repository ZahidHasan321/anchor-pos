...
{#snippet createUserForm()}
	<form action="?/create" method="POST" use:enhance enctype="multipart/form-data" class="space-y-4">
...
{:else}
	<Sheet.Root bind:open={isCreateDialogOpen}>
		<Sheet.Content side="bottom" class="max-h-[85vh] rounded-t-2xl">
			<Sheet.Header>
				<Sheet.Title>Create New User</Sheet.Title>
				<Sheet.Description>Add a new staff member to the system.</Sheet.Description>
			</Sheet.Header>
...
{:else}
	<Sheet.Root bind:open={isResetDialogOpen}>
		<Sheet.Content side="bottom" class="max-h-[85vh] rounded-t-2xl">
			<Sheet.Header>
