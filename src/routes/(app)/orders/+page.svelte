	...
	function handleBarcodeSearch(e: KeyboardEvent) {
		if (e.key === 'Enter' && searchQuery.trim()) {
			...
			const match = data.orders.find(
				(o) => o.orderNumber.toString() === query || o.id.toLowerCase() === query
			);
			if (match) {
	...
	<div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
		...
		<div class="relative w-full sm:w-96">
			...
			<Input
				...
				class="h-11 border-primary/50 pr-24 pl-10 shadow-sm focus-visible:ring-primary"
			...
			<div class="absolute top-1/2 right-3 flex -translate-y-1/2 items-center gap-2">
				...
				{/if}
				<div
					class="rounded bg-muted px-1.5 py-0.5 text-[10px] font-bold text-muted-foreground uppercase"
				>
					Enter
				</div>
			</div>
			...
			<div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
				...
				<div class="flex flex-1 flex-col gap-4 sm:flex-row sm:items-end">
					<div class="space-y-2">
						...
						<div class="flex items-center gap-2">
							<Input
								...
								class="h-9 w-full cursor-pointer text-xs sm:w-[140px]"
							/>
					...
					<div class="flex h-9 w-fit rounded-md border bg-muted/30 p-0.5">
						<Button
							...
							size="sm"
							onclick={setToday}
							class="h-full cursor-pointer px-3 text-[10px] sm:text-xs">Today</Button
						>
						...
						<Button
							...
							size="sm"
							onclick={setThisWeek}
							class="h-full cursor-pointer px-3 text-[10px] sm:text-xs">Week</Button
						>
						<Separator orientation="vertical" class="my-auto h-4" />
						<Button
							variant="ghost"
							...
							class="h-full cursor-pointer px-3 text-[10px] sm:text-xs">Month</Button
						>
					</div>
				</div>
				...
				<div class="flex items-end gap-2 sm:gap-3">
					<div class="flex-1 space-y-2 sm:flex-none">
						...
						>
							<Select.Trigger class="h-9 w-full text-xs sm:w-[140px]">
								{statusFilter
					...
					{#if dateFrom || dateTo || statusFilter || searchQuery}
						<Button
							...
							class="flex h-9 cursor-pointer gap-2 px-3 text-xs text-muted-foreground hover:text-destructive"
						>
							<X class="h-3.5 w-3.5" />
