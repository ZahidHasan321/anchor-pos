<script lang="ts">
	...
	$effect(() => {
		...
		Promise.all([
			...
			powersync.db.get(
				`SELECT coalesce(sum(amount), 0) as total FROM cashbook WHERE type = 'out' AND category = 'expense' AND created_at >= ?`,
				[todayIso]
			),
			powersync.db.get(
				`SELECT coalesce(sum(oi.cost_at_sale * oi.quantity), 0) as total FROM order_items oi INNER JOIN orders o ON oi.order_id = o.id WHERE o.status = 'completed' AND oi.status = 'completed' AND o.created_at >= ?`,
				[todayIso]
			),
			powersync.db.get(
				`SELECT coalesce(sum(total_amount), 0) as total FROM orders WHERE status = 'completed' AND created_at >= ? AND created_at < ?`,
		...
		Promise.all([
			powersync.db.getAll(
				`SELECT p.id, p.name, pv.size, pv.stock_quantity as stock FROM product_variants pv INNER JOIN products p ON pv.product_id = p.id WHERE pv.stock_quantity > 0 AND pv.stock_quantity <= 5 LIMIT 10`
			),
			powersync.db.get(
				`SELECT count(*) as count FROM product_variants WHERE stock_quantity > 0 AND stock_quantity <= 5`
			)
		]).then(([items, count]) => {
		...
		`,
			...
			.then((products: any) => (nativeTopProducts = products as any[]));
		...
		`,
			...
			.then((rows: any) => (nativeLast7Days = rows as any[]));

		// Payment breakdown
		Promise.all([
			powersync.db.get(
				`SELECT 'cash' as method, coalesce(sum(coalesce(cash_amount, CASE WHEN payment_method = 'cash' THEN total_amount ELSE 0 END)), 0) as total FROM orders WHERE status = 'completed' AND created_at >= ? AND (coalesce(cash_amount, 0) > 0 OR payment_method = 'cash')`,
				[todayIso]
			),
			powersync.db.get(
				`SELECT 'card' as method, coalesce(sum(coalesce(card_amount, CASE WHEN payment_method = 'card' THEN total_amount ELSE 0 END)), 0) as total FROM orders WHERE status = 'completed' AND created_at >= ? AND (coalesce(card_amount, 0) > 0 OR payment_method = 'card')`,
				[todayIso]
			),
			powersync.db.get(
				`SELECT 'mobile' as method, coalesce(sum(coalesce(mobile_amount, CASE WHEN payment_method = 'mobile' THEN total_amount ELSE 0 END)), 0) as total FROM orders WHERE status = 'completed' AND created_at >= ? AND (coalesce(mobile_amount, 0) > 0 OR payment_method = 'mobile')`,
				[todayIso]
			)
...
<div class="space-y-6 p-4 sm:p-6">
	...
	{#snippet kpiCards(stats: any)}
		{@const todayVsYesterday = comparePercent(
			stats.todaySales?.total ?? 0,
			stats.yesterdaySales?.total ?? 0
		)}
		{@const monthVsLast = comparePercent(
			stats.monthlySales?.total ?? 0,
			stats.lastMonthSamePeriod?.total ?? 0
		)}
		<div class="grid gap-2.5 sm:grid-cols-3 sm:gap-4">
			...
			>
				<div class="flex items-center justify-between">
					<span class="text-[10px] font-bold text-muted-foreground uppercase">Today's Sales</span>
					<div class="hidden rounded-md bg-emerald-500/10 p-1.5 sm:block">
						<ShoppingBag class="h-3.5 w-3.5 text-emerald-600" />
					</div>
				</div>
				<div class="mt-1.5 text-lg font-black break-all text-emerald-600 sm:mt-2 sm:text-2xl">
				...
				<div class="mt-0.5 flex items-center gap-2 sm:mt-1">
					...
					{#if todayVsYesterday.value > 0}
						<span
							class="inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[10px] font-medium {todayVsYesterday.up
								? 'bg-emerald-500/10 text-emerald-600'
								: 'bg-red-500/10 text-red-600'}"
						>
							{#if todayVsYesterday.up}<ArrowUpRight class="h-3 w-3" />{:else}<ArrowDownRight
									class="h-3 w-3"
								/>{/if}
			...
			>
				<div class="flex items-center justify-between">
					<span class="text-[10px] font-bold text-muted-foreground uppercase">Net Profit</span>
					<div class="hidden rounded-md bg-emerald-500/10 p-1.5 sm:block">
						<Wallet class="h-3.5 w-3.5 text-emerald-600" />
					</div>
				</div>
				<div
					class="mt-1.5 text-lg font-black break-all sm:mt-2 sm:text-2xl {stats.todayProfit >= 0
						? 'text-emerald-600'
						: 'text-red-600'}"
				>
			...
			>
				<div class="flex items-center justify-between">
					<span class="text-[10px] font-bold text-muted-foreground uppercase">Monthly Sales</span>
					<div class="hidden rounded-md bg-blue-500/10 p-1.5 sm:block">
						<TrendingUp class="h-3.5 w-3.5 text-blue-600" />
					</div>
				</div>
				<div class="mt-1.5 text-lg font-black break-all text-blue-600 sm:mt-2 sm:text-2xl">
				...
				<div class="mt-0.5 flex items-center gap-2 sm:mt-1">
					...
					{#if monthVsLast.value > 0}
						<span
							class="inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[10px] font-medium {monthVsLast.up
								? 'bg-emerald-500/10 text-emerald-600'
								: 'bg-red-500/10 text-red-600'}"
						>
							{#if monthVsLast.up}<ArrowUpRight class="h-3 w-3" />{:else}<ArrowDownRight
									class="h-3 w-3"
								/>{/if}
			...
			<Card.Content>
				{#if isNative}
					{#if nativeTodayPayments.length === 0 && nativeStats === null}
						<Skeleton class="h-[200px] w-full" />
					{:else if nativeTodayPayments.every((p: any) => Number(p?.total ?? 0) === 0)}
						<div class="flex h-[200px] flex-col items-center justify-center text-center">
							...
							<p class="text-sm font-medium text-muted-foreground">No sales yet today</p>
							<p class="mt-1 text-xs text-muted-foreground/60">Payment breakdown will appear after the first sale</p>
						</div>
					{:else}
						<div class="h-[200px]">
				...
				{:else}
					{#await data.todayPayments}
						<Skeleton class="h-[200px] w-full" />
					{:then payments}
						{#if payments.every((p: any) => Number(p?.total ?? 0) === 0)}
							<div class="flex h-[200px] flex-col items-center justify-center text-center">
							<div class="mb-2 rounded-full bg-muted p-3">
								<Wallet class="h-5 w-5 text-muted-foreground/40" />
							</div>
								...
								<Table.Body>
									{#each nativeTopProducts as product, i}
										<Table.Row>
											...
											<Table.Cell class="py-2.5 text-right font-mono text-xs sm:text-sm"
												>{product.totalQty}</Table.Cell
											>
											<Table.Cell
												class="py-2.5 pr-4 text-right text-xs font-bold sm:pr-0 sm:text-sm"
										...
										<Table.Row>
											<Table.Cell colspan={3}>
												<div class="flex flex-col items-center justify-center py-10 text-center">
													<div class="mb-2 rounded-full bg-muted p-3">
														<Trophy class="h-5 w-5 text-muted-foreground/40" />
													</div>
													<p class="text-sm font-medium text-muted-foreground">No sales this month yet</p>
													<p class="mt-1 text-xs text-muted-foreground/60">Products will appear here as sales come in</p>
												</div>
											</Table.Cell>
										</Table.Row>
									{/if}
								...
								<Table.Body>
									...
									<Table.Row>
										<Table.Cell colspan={2}>
											<div class="flex flex-col items-center justify-center py-10 text-center">
												<div class="mb-2 rounded-full bg-emerald-500/10 p-3">
													<Package class="h-5 w-5 text-emerald-600/50" />
												</div>
												<p class="text-sm font-medium text-muted-foreground">Stock levels healthy</p>
												<p class="mt-1 text-xs text-muted-foreground/60">All items are above the low stock threshold</p>
											</div>
										</Table.Cell>
									</Table.Row>
									{/if}
