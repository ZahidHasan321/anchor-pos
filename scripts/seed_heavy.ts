...
const FIRST_NAMES = ['John', 'Jane', 'Michael', 'Emily', 'Chris', 'Sarah', 'David', 'Anna', 'Robert', 'Linda', 'William', 'Elizabeth', 'James', 'Barbara', 'Richard', 'Susan', 'Joseph', 'Jessica', 'Thomas', 'Sarah'];
...
async function seed() {
    ...
    for (let i = 0; i < userRoles.length; i++) {
        ...
        const existing = client.prepare('SELECT id FROM user WHERE username = ?').get(username) as { id: string } | undefined;
        ...
        if (existing) {
            ...
            db.insert(schema.users).values({
                id: userId,
                username: username,
                passwordHash: hashPassword(i === 0 ? 'admin123' : 'password123'),
                role: role as any,
    ...
    const variantIds: { id: string, productId: string, price: number, productName: string, variantLabel: string }[] = [];
    for (let i = 0; i < 200; i++) {
        ...
        for (let j = 0; j < numVariants; j++) {
            ...
            // Initial stock log
            db.insert(schema.stockLogs).values({
                id: generateId(),
                variantId,
                changeAmount: stock,
                reason: 'restock',
    ...
    for (let i = 0; i < 2000; i++) {
        ...
        for (let j = 0; j < numItems; j++) {
            ...
            client.prepare('UPDATE product_variant SET stock_quantity = stock_quantity - ? WHERE id = ?').run(quantity, variant.id);
        ...
        db.insert(schema.cashbook).values({
            id: generateId(),
            amount: totalAmount,
            type: 'in',
